...
public class UnicodeEscapingTest extends GWTTestCase {
  ...
  public static void verifyStringContainingCharacterRange(int start, int end,
    ...
    for (int i = 0, codePoint = start; i < strLen;
      ...
      if (strCodePoint != codePoint) {
        throw new InvalidCharacterException(i, start, end, codePoint,
            strCodePoint);
      }
      ++codePoint;
    }
    if (strLen != expectedLen) {
      throw new InvalidCharacterException(-1, start, end, expectedLen, strLen);
    }
  }
   ...
   * 
   * Unpaired surrogates are not tested here as some browsers will not accept
   * them. Properly paired surrogates are tested in the non-BMP test.
   * 
